...
open public class PathFindingNodeCost : PathFindingNode, Comparable<PathFindingNodeCost> {

    companion object {

        val NULL_PATH_FINDING_NODE_COST_ARRAY_ARRAY: Array<Array<PathFindingNodeCost?>?> =
            Array(0) { arrayOfNulls<PathFindingNodeCost?>(0) }
    }
    ...
    override fun compareTo(
        pathFindingNodeCost: PathFindingNodeCost
    )
        // nullable = true from not(false or (false and false)) = true
        : Int {
        ...
        // if statement needs to be on the same line and ternary does not work the same way.
        return this.pathFindingNodeCostInfoP!!.compareTo(
            pathFindingNodeCost!!.pathFindingNodeCostInfoP
        )
    }
