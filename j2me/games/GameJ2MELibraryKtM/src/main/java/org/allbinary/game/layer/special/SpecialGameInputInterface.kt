...
interface SpecialGameInputInterface : PaintableInterface {
    ...
    @Throws(Exception::class) open fun up()

    // nullable = true from not(false or (false and true)) = true

    @Throws(Exception::class) open fun down()

    // nullable = true from not(false or (false and true)) = true

    @Throws(Exception::class) open fun right()

    // nullable = true from not(false or (false and true)) = true

    @Throws(Exception::class) open fun left()

    // nullable = true from not(false or (false and true)) = true

    @Throws(Exception::class) open fun strafeLeft()

    // nullable = true from not(false or (false and true)) = true

    @Throws(Exception::class) open fun strafeRight()
