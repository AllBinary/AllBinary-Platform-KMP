...
open public class AnimationFactoryImageScaleUtil : Object {
        ...
        // nullable = true from not(false or (false and false)) = true
    {
        ...
        if (scaleProperties!!.scaleWidth != 0 && scaleProperties!!.scaleHeight != 0) {
            ...
            var animationFactoryInitializationVisitor: AnimationFactoryInitializationVisitor =
                baseImageAnimationFactory!!.getAnimationFactoryInitializationVisitorP()!!

            var scaleX: Float =
                (scaleProperties!!.scaleWidth.toFloat()) /
                    (animationFactoryInitializationVisitor!!.width.toFloat())

            var scaleY: Float =
                (scaleProperties!!.scaleHeight.toFloat()) /
                    (animationFactoryInitializationVisitor!!.height.toFloat())

            if ((scaleX == 1.0f && scaleY == 1.0f) || (scaleX == 0.0f || scaleY == 0.0f)) {} else {

                animationFactoryInitializationVisitor!!.dx =
                    (animationFactoryInitializationVisitor!!.originalDx * scaleX).toInt()
                animationFactoryInitializationVisitor!!.dy =
                    (animationFactoryInitializationVisitor!!.originalDy * scaleY).toInt()
            }
        } else {}
    }
}
