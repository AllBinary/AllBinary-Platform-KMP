...
open public class FeaturedAnimationInterfaceFactoryInterfaceFactory : FeaturedResourceFactory {
    ...
    companion object {
        ...
        private var INSTANCE: FeaturedAnimationInterfaceFactoryInterfaceFactory =
    ...
    open fun getRectangleXY(
        ...
        for (index in 0 until size) {
            ...
            var featureInterface: FeatureResourceAnimationInterfaceFactoryInterface =
                list.objectArray[index]!! as FeatureResourceAnimationInterfaceFactoryInterface
            ...
            if (featureInterface!!.isFeature()) {
                ...
                if (rectangle != RectangleFactory.SINGLETON) {
                    ...
                    // way.
                    return Rectangle(
                        pointFactory!!.createXY(x, y),
                        ((rectangle.getWidth() * scale) shr 1),
                        ((rectangle.getHeight() * scale) shr 1),
                    )
                }
            }
        }
        ...
        throw Exception(
            StringMaker()
                .append("No rectangle available for current feature selection or Resource: ")!!
                .append(resource)!!
                .toString()
        )
    }
    ...
    @Throws(Exception::class)
    open fun getBasicAnimationInterfaceFactoryInstance(
        resource: String
    )
        // nullable = true from not(false or (false and false)) = true
        ...
        for (index in 0 until size) {
            ...
            if (featureInterface!!.isFeature()) {
                ...
                if (
                    animationInterfaceFactoryInterface !=
                        NullAnimationFactory.NULL_NOT_FOR_USE_ANIMATION_FACTORY
                ) {
        ...
        if (resourceTypeAvailableList!!.size() > 0) {
            ...
            throw Exception(
                stringBuffer!!
                    .append("No feature resource type available for Resource: ")!!
                    .append(resource)!!
                    .append(" Resource Factories Available: ")!!
                    .append(result)!!
                    .toString()
            )
        }
