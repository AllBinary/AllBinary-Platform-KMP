...
open public class AbFile : Object, AbFileInterface {

    companion object {

        val NULL_FILE: AbFile =
            AbFile.createAbFileFromRawPath(StringUtil.getInstance()!!.EMPTY_STRING)!!

        @Throws(Exception::class)
        open fun createAbFileWithChild(
            file: AbFile,
        ...
        open fun createAbFile(
            ...
            // if statement needs to be on the same line and ternary does not work the same way.
            return AbFile(
                File(AbPath(filePath, StringUtil.getInstance()!!.EMPTY_STRING).toFileSystemString())
            )
        }

        @Throws(Exception::class)
        open fun createAbFilePathAndName(
            filePath: String,
            fileName: String,
        )
            // nullable = true from not(false or (false and false)) = true
            ...
            return AbFile(
                File(
                    AbPath(filePath, StringUtil.getInstance()!!.EMPTY_STRING).toFileSystemString(),
                    fileName,
                )
            )
        }

        open fun createAbFileFromAbPath(
            abPath: AbPath
        )
            // nullable = true from not(false or (false and false)) = true
            : AbFile {
        ...
        open fun createAbFileFromRawPath(
            filePath: String
        )
            // nullable = true from not(false or (false and false)) = true
            : AbFile {
    ...
    protected constructor(file: File) : super() {
        // var file = file
        this.file = file
    }
    ...
    open fun listFilesFileNameFilter(
        filter: FilenameFilter
    )
        // nullable = true from not(false or (false and false)) = true
        : Array<Any?> {
