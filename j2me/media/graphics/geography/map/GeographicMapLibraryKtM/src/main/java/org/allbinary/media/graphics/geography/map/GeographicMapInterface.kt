...
interface GeographicMapInterface {
    ...
    @Throws(Exception::class)
    open fun getCellPositionAtXYNoThrow(
        x: Int,
        y: Int,
    )
        // nullable = true from not(false or (false and false)) = true
    ...
    open fun getCellTypeFromMapCellTypeInt(
        cellTypeId: Int
    )
        // nullable = true from not(false or (false and false)) = true
        : Int
