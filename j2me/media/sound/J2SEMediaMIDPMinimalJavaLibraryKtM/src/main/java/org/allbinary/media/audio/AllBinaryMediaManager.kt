    ...
    companion object {
        ...
        open fun createPlayer(
            ...
            if (resource.startsWith(Manager.TONE_DEVICE_LOCATOR)) {
                ...
                // if statement needs to be on the same line and ternary does not work the same way.
                return AllBinaryMediaManager.createPlayerFromInputStream(
                    ByteArrayInputStream(NullUtil.getInstance()!!.NULL_BYTE_ARRAY),
                    AudioContentTypeDataFactory.getInstance()!!.MIME_AUDIO_TONE.getName(),
                )
            } else if (
                Features.getInstance()!!.isFeature(GameFeatureFactory.getInstance()!!.SOUND)
            ) {
