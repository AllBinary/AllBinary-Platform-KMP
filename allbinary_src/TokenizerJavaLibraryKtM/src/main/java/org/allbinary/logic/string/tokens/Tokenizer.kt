...
open public class Tokenizer : Object {
        ...
        while (index < size) {
            ...
            if (index != -1) {
                ...
                if (end != -1) {
                    ...
                    tokenList!!.add(
                        string.substring(index + this.sep.length, end - (this.endSep!!.length - 1))
                    )
    ...
    open fun getTokensFromString(
        string: String,
        tokenVector: BasicArrayList,
    )
        // nullable = true from not(false or (false and false)) = true
        : BasicArrayList {
        var string = string
