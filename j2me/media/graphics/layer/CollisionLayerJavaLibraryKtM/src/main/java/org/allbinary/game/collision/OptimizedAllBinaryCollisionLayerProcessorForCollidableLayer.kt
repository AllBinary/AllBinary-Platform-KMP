...
open public class OptimizedAllBinaryCollisionLayerProcessorForCollidableLayer : LayerProcessor {
    ...
    public constructor() : super(OptimizedAllBinaryCollisionLayerManager()) {
        ...
        // nullable = true from not(false or (false and false)) = true
    {
        ...
        var collidableInterfaceCompositeInterface: CollidableCompositeLayer =
            layerInterface as CollidableCompositeLayer
        ...
        if (collidableBase!!.isCollidable(collidableInterfaceCompositeInterface)) {
            ...
            for (index in startIndex + 1 until size) {
                ...
                collidableInterfaceCompositeInterface2 =
                    layerManager!!.getLayerAt(index) as CollidableCompositeLayer
                collidableBase2 = collidableInterfaceCompositeInterface2!!.getCollidableInferface()

                if (
                    collidableBase2!!.isCollidable(collidableInterfaceCompositeInterface) &&
                        collidableBase!!.isCollision(
                            collidableInterfaceCompositeInterface,
                            collidableInterfaceCompositeInterface2,
                        )
                ) {
                    collidableBase!!.collide(
                        collidableInterfaceCompositeInterface,
                        collidableInterfaceCompositeInterface2,
                    )
                    collidableBase2!!.collide(
                        collidableInterfaceCompositeInterface2,
                        collidableInterfaceCompositeInterface,
                    )
                }
