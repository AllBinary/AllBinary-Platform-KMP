...
open public class PathFindingNodeCostInfoFactory : PathFindingNodeCostInfoFactoryBase {
    ...
    public constructor(max: Int) {
        ...
        this.pathFindingNodeCostInfoAdjacencyList =
            Array(max) { arrayOfNulls<PathFindingNodeCostInfo?>(max) }
    }

    @Throws(Exception::class)
    override fun create(
        geographicMapInterface: BasicGeographicMap,
        goingToGeographicMapCellPosition: GeographicMapCellPosition,
    ...
    open fun getInstance(
        ...
        if (pathFindingNodeCostInfo == null) {
            ...
            this.pathFindingNodeCostInfoAdjacencyList[geographicMapCellPosition!!.getId()]!![
                goingToGeographicMapCellPosition!!.getId()] = pathFindingNodeCostInfo
        } else {
    ...
    open fun getInstance(
        goingToFromGeographicMapCellPosition: GeographicMapCellPosition,
        geographicMapCellPosition: GeographicMapCellPosition,
    )
        // nullable =  from not(true or (false and false)) =
        : PathFindingNodeCostInfo {
        ...
        // if statement needs to be on the same line and ternary does not work the same way.
        return this.pathFindingNodeCostInfoAdjacencyList[geographicMapCellPosition!!.getId()]!![
            goingToFromGeographicMapCellPosition!!.getId()]!!
    }

    override fun getTotalCost(
        geographicMapInterface: BasicGeographicMap,
        comingFromGeographicMapCellPosition: GeographicMapCellPosition,
        geographicMapCellPosition: GeographicMapCellPosition,
    )
        // nullable = true from not(false or (false and false)) = true
        : Long {
        ...
        // if statement needs to be on the same line and ternary does not work the same way.
        return this.getInstance(comingFromGeographicMapCellPosition, geographicMapCellPosition)!!
            .totalCostP
    }
}
