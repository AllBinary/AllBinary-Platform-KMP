...
open public class BasicGeographicMap : SimpleGeographicMap, GeographicMapInterface {
    ...
    public constructor(
        id: Integer,
        name: String,
        cellTypeIdToGeographicMapCellType: IntArray,
    ...
    open fun getCellPosition(
        ...
        when (direction) {
            0 -> {
                ...
                return this.geographicMapCellPositionFactory!!.getAt(
                    oldGeographicMapCellPosition!!.getColumn() - 1,
                    oldGeographicMapCellPosition!!.getRow(),
                )
            }
            1 -> {
            ...
            2 -> {
                ...
                // if statement needs to be on the same line and ternary does not work the same way.
                return this.geographicMapCellPositionFactory!!.getAt(
                    oldGeographicMapCellPosition!!.getColumn(),
                    oldGeographicMapCellPosition!!.getRow() - 1,
                )
            }
            3 -> {
    ...
    @Throws(Exception::class)
    open fun getCellPositionNoThrow(
        direction: Int,
        oldGeographicMapCellPosition: GeographicMapCellPosition,
    )
        // nullable = true from not(false or (false and false)) = true
        ...
        when (direction) {
            ...
            1 -> {
                ...
                if (
                    oldGeographicMapCellPosition!!.getColumn() + 1 <
                        this.getAllBinaryTiledLayer()!!.getColumns()
                ) {
            ...
            3 -> {
                ...
                if (
                    oldGeographicMapCellPosition!!.getRow() + 1 <
                        this.getAllBinaryTiledLayer()!!.getRows()
                ) {
    ...
    @Throws(Exception::class)
    open fun isOfFourDirections(
        oldGeographicMapCellPosition: GeographicMapCellPosition,
        newGeographicMapCellPosition: GeographicMapCellPosition,
    )
        // nullable = true from not(false or (false and false)) = true
        : Boolean {
        // var oldGeographicMapCellPosition = oldGeographicMapCellPosition
        ...
        for (index in 0 until 4) {
            ...
            if (
                newGeographicMapCellPosition ==
                    this.getCellPositionNoThrow(index, oldGeographicMapCellPosition)
            ) {
    ...
    @Throws(Exception::class)
    open fun getCellPositionAtNoThrow(
        x: Int,
        y: Int,
        x2: Int,
        y2: Int,
        geographicMapCellPositionList: BasicArrayList,
    )
        // nullable = true from not(false or (false and false)) = true
        : BasicArrayList {
        // var x = x
        ...
        for (columnIndex in i_columnMin until i_columnMax) {
            ...
            for (rowIndex in i_rowMin until i_rowMax) {
                ...
                if (
                    allBinaryTiledLayer!!.getColumns() > columnIndex &&
                        allBinaryTiledLayer!!.getRows() > rowIndex
                ) {
                    geographicMapCellPositionList!!.add(
                        this.geographicMapCellPositionFactory!!.getAt(columnIndex, rowIndex)
                    )
                }
            }
        }
    ...
    override fun getCellPositionsAt(
        ...
        for (index in 0 until size) {
            ...
            for (index2 in 0 until size2) {
                ...
                if (
                    currentCellPositionArray[index]!![index2] != cellPositionArray[index]!![index2]
                ) {
                    hasChanged = true
                }
            }
